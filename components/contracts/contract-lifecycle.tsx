...
export function ContractLifecycle({ contractId, status, snapshot, defaultEmail }: Readonly<{ contractId: string; status: ContractStatus; snapshot: ContractSnapshot; defaultEmail?: string | null }>) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="lifecycle-title"><h2 id="lifecycle-title" className="text-lg font-semibold text-slate-950">Operações</h2><div className="mt-4 space-y-5">
    ...
    {(status === "GENERATED" || status === "SENT") ? <div className="border-t border-slate-200 pt-5"><p className="text-sm text-slate-600">O cancelamento preserva snapshot, documentos e histórico.</p>{confirmingCancel ? <div className="mt-3 rounded-lg border border-red-200 bg-red-50 p-4"><p className="font-semibold text-red-950">Cancelar este Contrato?</p><p className="mt-1 text-sm text-red-900">Esta ação encerra o Contrato e não pode ser revertida.</p><div className="mt-4 flex flex-wrap gap-3"><button type="button" disabled={pending !== null} onClick={() => setConfirmingCancel(false)} className="min-h-11 rounded-lg border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-700">Voltar</button><button type="button" disabled={pending !== null} onClick={() => run("cancel", () => cancelContractAction({ contract_id: contractId }))} className="min-h-11 rounded-lg bg-red-700 px-4 text-sm font-semibold text-white disabled:opacity-60">{pending === "cancel" ? "Cancelando..." : "Confirmar cancelamento"}</button></div></div> : <button type="button" disabled={pending !== null} onClick={() => setConfirmingCancel(true)} className="mt-3 min-h-11 rounded-lg border border-red-300 px-4 text-sm font-semibold text-red-700 disabled:opacity-60">Cancelar Contrato</button>}</div> : null}
