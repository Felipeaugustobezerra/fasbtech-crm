...
export function FinancialEntryForm({ mode, entryId, clients, initialValues }: Props) {
  ...
  return <form onSubmit={handleSubmit(onSubmit)} noValidate aria-busy={isSubmitting} className="space-y-6"><fieldset className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><legend className="px-1 text-base font-semibold text-slate-950">Dados da movimentação</legend><div className="mt-3 grid gap-5 sm:grid-cols-2">
    ...
    <label className="text-sm font-medium text-slate-700 sm:col-span-2">Descrição <span className="text-red-700" aria-hidden="true">*</span><input required className={inputClass} aria-invalid={Boolean(fieldError("description"))} aria-describedby={fieldError("description") ? "financial-description-error" : undefined} {...register("description")} />{fieldError("description") ? <span id="financial-description-error" className="mt-1 block text-sm text-red-700">{fieldError("description")}</span> : null}</label>
    <label className="text-sm font-medium text-slate-700">Valor (EUR) <span className="text-red-700" aria-hidden="true">*</span><input required inputMode="decimal" placeholder="0.00" className={inputClass} aria-invalid={Boolean(fieldError("amount"))} aria-describedby={fieldError("amount") ? "financial-amount-error" : undefined} {...register("amount")} />{fieldError("amount") ? <span id="financial-amount-error" className="mt-1 block text-sm text-red-700">{fieldError("amount")}</span> : null}</label>
    <label className="text-sm font-medium text-slate-700">Data de referência <span className="text-red-700" aria-hidden="true">*</span><input required type="date" className={inputClass} aria-invalid={Boolean(fieldError("reference_date"))} aria-describedby={fieldError("reference_date") ? "financial-reference-date-error" : undefined} {...register("reference_date")} />{fieldError("reference_date") ? <span id="financial-reference-date-error" className="mt-1 block text-sm text-red-700">{fieldError("reference_date")}</span> : null}</label>
